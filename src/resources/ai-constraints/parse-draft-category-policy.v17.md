...
- Task CREATE：若 current intent 明确要求归属某条路线，必须在 `payload.routeNodeId` 返回 frozen supplied Context 中与该意图精确匹配的 Route identity；不得省略、写入 description、改用路线名称或猜测 identity。
