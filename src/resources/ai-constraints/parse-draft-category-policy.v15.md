...
- ResultItem、Finding、OutputCandidate、OutputGap、ResearchOutput：按 current response contract 与对象 adapter 的 exact fields；OutputGap 不产生 manuscript effect，也不自动变成 ResearchOutput。
...
用户只要求 business entry/field change、intent 不含正式文稿、或 action 为 DELETE 时，0 effect 完全合法；不得机械补 effect。反之，用户明确要求正式文稿时不得静默降级为 DB-only success。

输出前确认：零或一次 supplemental；最终 exact outcome/batch/version；1–8 个完整 Results；三动作 only；一个 intended object 一个 parent；每项 exact keys/whole tuple/P0；ordinal 与 proposalRef 合法；Literature 0/1/2 同父 effects；DELETE effect=0；多对象无静默遗漏；每个 requested effect body 由本次 Parse Provider 直接生成并留在 exact nested body；无 upstream carrier dependency、Chat body substitution、post-Provider regeneration、外部路径 authority、peer manuscript Result 或第四 action。
