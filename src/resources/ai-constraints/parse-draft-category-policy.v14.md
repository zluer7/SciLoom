...
## 最高优先级：Literature 一父对象 / 0-1-2 channel effects

对一个 source Literature、一个 intended Literature object，或用户明确所说的“同一篇/同一个 Literature 对象”，必须且只能输出一个 Literature 父业务 Result。`literature_outline` 与 `dedicated_notes` 是该同一 owner 下的 subordinate formal manuscript channels；两个 channel、两个不同正文、两个不同 heading/title suffix 都不构成两个 Literature 对象或两个父动作。不得先输出两个 peer Literature proposals，再按相同 `intentRef`、相似标题或同一材料事后合并；这种 peer-parent shape 本身不合规。

CREATE Literature：parent action count = 1，canonical settlement 后 generated owner count = 1，父 `payload.manuscriptEffects` count = 0 | 1 | 2。UPDATE Literature：exact existing target count = 1，父 `payload.manuscriptEffects` count = 0 | 1 | 2。DELETE Literature：只针对 whole Literature object，不产生 channel-level delete。

显式用户意图优先于 AI 判断和 fallback：只要 outline 就只返回一个 `literature_outline` effect；只要 notes 就只返回一个 `dedicated_notes` effect；明确两个都要就各返回一个 effect；只有 channel 确实不明确时，AI 才可选择一个 channel 或同时返回两个。notes-only 时在当前 call 内完成必要的客观理解，不得额外返回 formal outline effect，也不得要求第二次 Provider call。

内容分离采用 Constraint-first / best-effort：`literature_outline` 以原文为事实基础，客观、中性组织，不写入用户研究背景、项目立场或个人解释；`dedicated_notes` 先建立必要的客观理解，可在有用时利用 existing objective outline，再结合 authorized research Context 形成用户专属笔记。

Correct：一个 Literature parent title 使用 source Literature identity/title；`literature_outline` heading 可为 `<source title> 客观纲要`，`dedicated_notes` heading 可为 `<source title> 专属笔记`；两个 body 都嵌套在同一个 parent proposal 的 `manuscriptEffects`。

Forbidden：分别输出 `CREATE Literature title = "<source title> 客观纲要"` 与 `CREATE Literature title = "<source title> 专属笔记"` 两个 peer parent proposals。channel-specific suffix 只属于 manuscript heading/metadata，不得提升为 Literature parent title。
...
保持 chronological order；后续明确纠正、否定、替换或撤回时以后续为准。一个 action × 一个 business object 对应一个 Result。当前 range 显式要求 N 个受支持的普通 business objects 时，必须按原始提及顺序返回 N 个 Result；不得合并、省略、替换、重排或静默丢弃任何一项。Literature 的两个 formal channels 始终计为同一个 Literature business object，而不是 N=2。
