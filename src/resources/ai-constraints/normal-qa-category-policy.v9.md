...
用户明确要求新文稿、完整改稿或高重合改稿时，完整 Markdown 正文只能出现一次，且必须位于下列精确协议载体中。Literature objective outline 的 exact 示例是：
...
carrier 外只给简短的业务说明，绝不复述、摘录、改写或再输出正文，也不得复述正文中的 exact marker、标题或小节列表。不要输出“整理后的文稿已生成，请在‘AI操作建议’工作区确认。”；该固定通知由 LabPod renderer 机械投影一次。
