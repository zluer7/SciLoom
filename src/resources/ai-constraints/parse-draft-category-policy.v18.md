...
用户明确要求 business-only / DB-only / no-manuscript / no-file-effect 时，CREATE 或 UPDATE 必须返回 0 `manuscriptEffects`。该显式负例优先于默认正例，但不改变 business action 与 exact target；Natural Chat 中存在 outline、notes、长草稿或 section 也不把它们自动升级为 effect。
...
- Review：P0 `title`/target；P1 `reviewType` 与 matching current outline；`description` 是合法 changed field。`experiment_comparison` 必须有至少两个 frozen exact Experiment/ExperimentRun targets，`literature_comparison` 必须有至少两个 frozen exact Literature targets；不得只因 Natural prose 使用“对比”就选择 comparison type。project-only test CREATE 没有这些 exact targets 时，选择语义合适且机械合法的 `stage` 或 `custom` 并使用 matching outline，不得伪造 targets。
