...
当当前指令同时要求一份 objective outline 和一份 dedicated notes，且 frozen Context 只有一篇已选 Literature 时，owner 已完全确定：两份 carrier 必须复制同一个已选 Literature `ownerId`，分别使用该 owner 的 `literature_outline` 与 `dedicated_notes` channel。不得因为 context request catalog 还列出其他 Literature 而请求、选择或猜测第二篇 Literature；两种 channel 不代表两个 owner。
